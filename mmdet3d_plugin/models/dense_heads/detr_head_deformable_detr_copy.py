...
class DETRHeadCustom(BaseModule):
    ...
    def __init__(
        ...
        if class_weight is not None and (self.__class__ is DETRHeadCustom):
    ...
    def loss(self, hidden_states: Tensor,
        ...
        # if nan then replace with zeros to avoid contamination
        outs_safe = ()
        for i_out, out in enumerate(outs):
            if torch.isnan(out).any() or torch.isinf(out).any():
                out_safe = torch.nan_to_num(out, nan=0.0, posinf=0.0,
                                              neginf=0.0)
                outs_safe += (out_safe,)
    ...
    def loss_by_feat_single(self, cls_scores: Tensor, bbox_preds: Tensor,
        ...
        if isinstance(self.loss_cls, QualityFocalLoss):
            ...
            loss_cls = self.loss_cls(
                cls_scores, labels.to(cls_scores.device), label_weights.to(cls_scores.device), avg_factor=cls_avg_factor)
    ...
    def _get_targets_single(self, cls_score: Tensor, bbox_pred: Tensor,
        ...
        pos_gt_bboxes_normalized = pos_gt_bboxes.to(factor.device) / factor
    ...
    def _predict_by_feat_single(self,
        ...
        # for each box add scores at the end such that shape of each box is n, 5
